...
impl ReissueTransaction {
    ...
    pub async fn validate<K: KeyManager>(&self, verifier: &K) -> Result<()> {
    ...
    async fn validate_input_dbcs<K: KeyManager>(&self, verifier: &K) -> Result<()> {
...
impl<K: KeyManager> Mint<K> {
    ...
    pub async fn reissue(
        ...
        reissue_req.transaction.validate(self.key_manager()).await?;
...
mod tests {
    ...
    fn prop_genesis() {
        ...
        let validation = block(genesis_dbc.confirm_valid(genesis_node.key_manager()));
    ...
    fn prop_splitting_the_genesis_dbc(output_amounts: TinyVec<TinyInt>) {
        ...
        for dbc in output_dbcs.iter() {
            ...
            assert!(block(dbc.confirm_valid(&key_manager)).is_ok());
        }
    ...
    fn prop_dbc_transaction_many_to_many(
        ...
        match many_to_many_result {
            Ok((transaction, transaction_sigs)) => {
                ...
                for dbc in output_dbcs.iter() {
                    let dbc_confirm_result = block(dbc.confirm_valid(&genesis_node.key_manager));
